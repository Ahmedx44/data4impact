...
class AppLogger {
  ...
  /// The app name.
  static String appName = 'marakipos';

  /// The map of log files.
  static final Map<LogType, File> logFiles = {};

  /// Initializes the logger.
  static void initialize({bool verbose = false, String appName = 'marakipos'}) {
    log = Logger(
  ...
  static Future<File> getLogsPath(LogType type) async {
    ...
    final file = File(join(dir, '${appName}-${type.name}.log'));
  ...
  static Future<String> _getLogDirectory() async {
    ...
    if (Platform.isLinux) {
      ...
      if (homeDir != null) {
        customPath = '$homeDir/.$appName/logs';
      }
      ...
      customPath = '${directory.path}/.$appName/logs';
    } else if (Platform.isAndroid) {
      final directory = await getApplicationDocumentsDirectory();
      // final directory = await getExternalStorageDirectory();
      // final directory = Platform.environment['EXTERNAL_STORAGE'];
      customPath = '${directory.path}/.$appName/logs';
      // customPath = '$directory/.$appName/logs';
    }
